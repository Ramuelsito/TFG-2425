...
Solution SwapInter::GenerateEnvironment() {
  ...
  //         Task first_task_to_insert = new_machines[i].getTasksAssigned()[j];
  //         Task second_task_to_insert = new_machines[k].getTasksAssigned()[l];
  //         new_machines[i].RemoveTask(j, setup_times);
  //         new_machines[k].RemoveTask(l, setup_times);
  //         new_machines[k].InsertTask(first_task_to_insert, l, setup_times);
