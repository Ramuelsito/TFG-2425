...
void Machine::ReInsertTask(const int& task_index, const int& new_task_position, const std::vector<std::vector<int>>& setup_times) {
  ...
  for (int i = 0; i < tasks_assigned_.size(); i++) {
    if (i == 0) {
      tij = tasks_assigned_[i].GetTime() + setup_times[tasks_assigned_[0].GetId() + 1][i + 1];
    } else {
      tij = tasks_assigned_[i].GetTime() + setup_times[tasks_assigned_[i - 1].GetId() + 1][i + 1];
    }
...
void Machine::RemoveTask(const int& task_index, const std::vector<std::vector<int>>& setup_times) {
  tasks_assigned_.erase(tasks_assigned_.begin() + task_index);
  // tc_time_ -= total_completion_times_[task_index];
  total_completion_times_.erase(total_completion_times_.begin() + task_index);
}

void Machine::InsertTask(const Task& task, const int& task_index, const std::vector<std::vector<int>>& setup_times) {
